...
def test_preset_comparison_lenses_6_categories():
    ...
    # 1. Young high-risk user matches Risky or Medium
    young_match = next(l for l in lenses_young if l["is_user_match"])
    assert young_match["lens_name"] in ["Medium", "Risky"]

    # 2. Senior low-risk user matches Safe
    senior_match = next(l for l in lenses_senior if l["is_user_match"])
    assert senior_match["lens_name"] == "Safe"

    # 3. Check 6 category breakdown present
